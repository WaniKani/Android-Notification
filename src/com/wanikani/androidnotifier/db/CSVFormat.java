...
public class CSVFormat implements Format {
		...
		throws IOException, SQLException
	{
		...
		levelups = invert (db.getLevelups ());
		...
		try {			
			...
			while (c.moveToNext ()) {				
				...
				newLevel = levelups.get (day);
				if (newLevel != null)
					level = newLevel;

				switch (HistoryDatabase.Facts.getType (c)) {
				case COMPLETE:
					dumpComplete (os, day, level, srs);
					break;
					
				case PARTIAL:
					dumpPartial (os, day, level, srs);
					break;
	...
	protected void dumpComplete (PrintStream os, int day, int level, SRSDistribution srs)
	{
		row (os, day, level,
	...
	protected void dumpPartial (PrintStream os, int day, int level, SRSDistribution srs)
	{
		row (os, day, level,
	...
	protected static Map<Integer, Integer> invert (Map<Integer, Integer> levelups)
	{
